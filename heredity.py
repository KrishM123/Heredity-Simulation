...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if mom is None and dad is None:
            ...
            prob *= (
                passing[mom] * passing[dad] if genes == 2 else
                passing[mom] * (1 - passing[dad]) + (1 - passing[mom]) * passing[dad] if genes == 1 else
...
def normalize(probabilities):
    for person in probabilities:
        for ele in probabilities[person]:
            tot = sum(dict(probabilities[person][ele]).values())
            for val in probabilities[person][ele]:
                probabilities[person][ele][val] /= tot
